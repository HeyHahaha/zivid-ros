...
protected:
  ...
  void comparePointCoordinate(float rosCoordinate, float sdkCoordinate) const
  {
...
TEST_F(CaptureOutputTest, testCapturePointsXYZGBA)
{
  ...
  const auto pointCloud = captureViaSDKDefaultSettings();
  const auto expectedXYZRGBA = pointCloud.copyData<Zivid::PointXYZColorRGBA>();
  ...
  {
    uint8_t* point_ptr = &last_pc2->data[i * last_pc2->point_step];
    ...
    const auto expectedARGB = static_cast<uint32_t>(expected.color.a << 24) |
...
TEST_F(CaptureOutputTest, testCapturePointsXYZ)
{
  ...
  auto pointCloud = captureViaSDKDefaultSettings();
  auto expectedXYZ = pointCloud.copyData<Zivid::PointXYZ>();
  ...
  {
    uint8_t* point_ptr = &point_cloud->data[i * point_cloud->point_step];
    const float x = *reinterpret_cast<float*>(&point_ptr[0]);
    const float y = *reinterpret_cast<float*>(&point_ptr[4]);
    const float z = *reinterpret_cast<float*>(&point_ptr[8]);
...
TEST_F(CaptureOutputTest, testCapture3DColorImage)
{
  ...
  const auto pointCloud = captureViaSDKDefaultSettings();
  const auto expectedRGBA = pointCloud.copyData<Zivid::ColorRGBA>();
...
{
  std::optional<sensor_msgs::Image> image;
  auto depth_image_sub = subscribe<sensor_msgs::Image>(depth_image_topic_name, [&](const auto& i) { image = *i; });
  ...
  const auto pointCloud = captureViaSDKDefaultSettings();
  const auto expectedZ = pointCloud.copyData<Zivid::PointZ>();
...
TEST_F(CaptureOutputTest, testCaptureSNRImage)
{
  ...
  const auto pointCloud = captureViaSDKDefaultSettings();
  const auto expectedSNR = pointCloud.copyData<Zivid::SNR>();
...
TEST_F(ZividNodeTest, testCaptureCameraInfo)
{
  ...
  std::optional<sensor_msgs::CameraInfo> snr_camera_info;
  auto snr_camera_info_sub =
      subscribe<sensor_msgs::CameraInfo>(snr_camera_info_topic_name, [&](const auto& r) { snr_camera_info = *r; });
  ...
  ASSERT_TRUE(snr_camera_info.has_value());
  assertCameraInfoForFileCamera(*snr_camera_info);
}
...
{
protected:
  ZividCATest() : m_camera(m_zivid.createFileCamera(file_camera_path)), m_cameraSettingsClient("/zivid_camera/settings")
  {
    waitForReady();
  ...
  zivid_camera::SettingsConfig settingsConfig()
  {
    ...
    EXPECT_TRUE(m_cameraSettingsClient.getCurrentConfiguration(cfg, dr_get_max_wait_duration));
    return cfg;
  }
...
private:
  Zivid::Application m_zivid;
  Zivid::Camera m_camera;
  dynamic_reconfigure::Client<zivid_camera::SettingsConfig> m_cameraSettingsClient;
